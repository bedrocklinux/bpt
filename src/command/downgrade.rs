...
pub fn downgrade(flags: CommonFlags, pkgs: Vec<PkgPathUrlRepo>) -> Result<String, Err> {
    ...
    let plan = {
        ...
        InstPkgReconciler {
            ...
            query_credentials,
            command: CommandRequest::Downgrade { pkgs: &pkgs },
        }
        .plan()?
    };
    ...
    let build_support = if plan.needs_builds() {
        ...
        Some(DowngradeBuildSupport {
            adjusted_root_dir: flags.root_dir.adjust_bedrock_prefix()?,
            // `downgrade` only builds ephemeral packages that are consumed immediately by the
