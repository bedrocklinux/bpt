...
pub fn install(
    ...
    let plan = {
        ...
        InstPkgReconciler {
            ...
            query_credentials,
            command: CommandRequest::Install {
                pkgs: &pkgs,
                reinstall,
            },
        }
    ...
    let build_support = if plan.needs_builds() {
        ...
        Some(InstallBuildSupport {
            adjusted_root_dir: flags.root_dir.adjust_bedrock_prefix()?,
            // `install` only builds ephemeral packages that are consumed immediately by the
