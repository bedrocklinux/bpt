...
fn build_from_path() {
    ...
    let stdout = run!("build", repo_path!("fakeblock.bbuild")).unwrap();
...
fn build_arch_native_rejected() {
    ...
    let result = run!("build", "-a", "native", repo_path!("fakeblock.bbuild"));
...
fn build_arch_bbuild_rejected() {
    ...
    let result = run!("build", "-a", "bbuild", repo_path!("fakeblock.bbuild"));
    assert!(result.is_err());
    let stderr = result.unwrap_err();
...
fn build_output_exists_rejected() {
    ...
    let result = run!("build", repo_path!("fakeblock.bbuild"));
    assert!(result.is_err());
    let stderr = result.unwrap_err();
...
fn build_with_makedepends_from_repo() {
    ...
    let stdout = run!("build", repo_path!("fakeblock-songs.bbuild")).unwrap();
...
fn build_orders_requested_bbuilds_by_makedepends() {
    ...
    let consumer = repo_path!("aaa-consumer.bbuild");
    let helper = repo_path!("zzz-helper.bbuild");
...
fn build_dry_run_from_path() {
    ...
    let stdout = run!("build", "-D", repo_path!("fakeblock.bbuild")).unwrap();
...
fn build_dry_run_output_exists_still_rejected() {
    ...
    let result = run!("build", "-D", repo_path!("fakeblock.bbuild"));
    assert!(result.is_err());
    let stderr = result.unwrap_err();
    assert!(stderr.contains("Cannot build `fakeblock@1.0.0:noarch.bpt`"));
    assert!(stderr.contains("as something already exists"));
...
fn build_arch_override_to_noarch_when_supported() {
    ...
    let stdout = run!("build", "-a", "x86_64", repo_path!("fakeblock.bbuild")).unwrap();
    assert!(stdout.contains("Built fakeblock@1.0.0:noarch.bpt"));
    assert!(std::path::Path::new(per_test_path!("fakeblock@1.0.0:noarch.bpt")).exists());
}
...
fn build_missing_makebins_errors() {
    ...
    write_modified_bbuild(
        repo_path!("fakeblock.bbuild"),
        per_test_path!("fakeblock-missing-makebin.bbuild"),
        &[(
            "makedepends=\"\"",
    ...
    let result = run!("build", per_test_path!("fakeblock-missing-makebin.bbuild"));
    assert!(result.is_err());
    let stderr = result.unwrap_err();
...
fn build_makebin_group_alias_expands_to_missing_bins() {
    ...
    write_modified_bbuild(
        repo_path!("fakeblock.bbuild"),
        per_test_path!("fakeblock-core-makebins.bbuild"),
    ...
    let result = run_at_root_with_path(
        per_test_path!(),
        &bindir,
        &["build", per_test_path!("fakeblock-core-makebins.bbuild")],
    );
    assert!(result.is_err());
    let stderr = result.unwrap_err();
    assert!(stderr.contains("required build-time binaries were not found in $PATH"));
